...
- [x] Guard against IDOR — e.g. a user can only list/view *their own* saved chats (DishLens), and DriveSync retrieval respects folder-level access scoping. DishLens half verified live: a second user's token against another user's chat ID returns the identical `404` as a nonexistent ID — no cross-user existence leak (`07-implementation-log.md` Cycle 9). DriveSync's `POST /sync/query` is now live (`07-implementation-log.md` Cycle 26) and requires a valid JWT, but there's no per-user/per-folder scoping to test yet — this project syncs a single shared Drive folder into a single shared Pinecone namespace (single-tenant by design, per `01-security-checklist.md` §4's own "not yet relevant" note on namespace isolation), so "any authenticated user can query all synced content" is the current, deliberate trust model, not a gap — the same trust boundary DishLens's own shared services assume.
...
- [x] Deleted Drive files trigger actual Pinecone vector deletion, not just a DB flag — stale vectors are a data leakage risk in retrieval results. `deleteVectorsForFile()` implemented and verified live against the real serverless index (Cycle 23), and **now actually triggered automatically** by `runSyncOnce()` for every ID in `detectChanges`'s `deletedFileIds` — verified live via a real BullMQ-triggered job run (`07-implementation-log.md` Cycle 25).
- [x] Sync job locking prevents concurrent runs from producing conflicting writes (BullMQ + Redis, see `06-toolchain-decisions.md`). `jobs/lock.ts`'s `acquireSyncLock`/`releaseSyncLock` (Redis `SET NX`/Lua compare-and-delete) wraps every sync run in `createSyncWorker`, on top of the `Worker`'s own `concurrency: 1`. Verified live: manually holding the lock caused a real triggered job to skip (logged `sync-skipped-locked`) rather than run concurrently (`07-implementation-log.md` Cycle 25).
...
- [x] Re-encode every uploaded image server-side (`sharp`) rather than storing the raw upload. `normalizeImage()` implemented + unit-tested (`07-implementation-log.md` Cycle 6) — now called on the live route (Cycle 15): only the normalized (re-encoded, EXIF-stripped) buffer is ever written to GCS, never the raw upload.
...
- [x] Upload to object storage (Google Cloud Storage) via pre-signed URLs rather than routing raw binary through the API server where avoidable (see `06-toolchain-decisions.md`). Implemented as a server-side upload of the normalized buffer plus pre-signed *read* URLs (`uploadNormalizedImage`/`getSignedReadUrl`, `07-implementation-log.md` Cycle 15) rather than a client-side pre-signed *upload* URL — this pipeline requires the server to inspect real bytes (blur/dimension/Vision) before a photo is ever accepted, so a direct-to-bucket client upload isn't avoidable-in-practice here; documented as a deliberate deviation from the literal checklist wording.
- [x] Use random, non-guessable object keys (UUID) — never the user-supplied filename. `${randomUUID()}.{jpg|png}`, unit-tested and verified live (`07-implementation-log.md` Cycle 15).
- [x] Keep any stored images private by default; signed, expiring URLs only. No `public`/ACL option set on upload; verified live that an unauthenticated fetch of the raw object URL returns `403`, while a v4 signed URL returns `200` (`07-implementation-log.md` Cycle 15).
