...
- [x] Deleted Drive files trigger actual Pinecone vector deletion, not just a DB flag — stale vectors are a data leakage risk in retrieval results. `deleteVectorsForFile()` implemented and verified live against the real serverless index: deleted exactly the target file's vectors, confirmed via a real `describeIndexStats()` count and a real `fetch()` proving an unrelated file's vector survived (`07-implementation-log.md` Cycle 23). Not yet *triggered automatically* on a real detected deletion — that wiring is Milestone #9's sync-job orchestration, still unstarted; this item covers the deletion mechanism itself.
