...
export async function generateRecipe(client: RecipeClient, model: string, dishName: string): Promise<RecipeGenerationResult> {
  const message = await client.messages.create({
    model,
    // 1024 was too tight: this model returns an unrequested `thinking` block
    // ahead of the actual JSON text, and the two together got cut off
    // mid-response (`stop_reason: "max_tokens"`) for recipes with a longer
    // ingredient list or a sub-recipe (e.g. a side sauce) - confirmed live
    // against a real dish (`_internal-docs/11-test-image-mapping.md`).
    max_tokens: 4096,
